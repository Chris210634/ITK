...
/*
  unsigned int tn = this->GetMultiThreader()->GetWorkerNumber();
  tn = (tn == 0) ? 1 : 0;
  if (tn*2 < this->m_NumberOfThreadsUsed)
    {
    std::ifstream ifs;
    this->GetMultiThreader()->GetIfstream(ifs, tn*2);
    this->ReadDataFromFile(ifs, tn*2);
    }
  if (tn*2+1 < this->m_NumberOfThreadsUsed)
    {
    std::ifstream ifs;
    this->GetMultiThreader()->GetIfstream(ifs, tn*2+1);
    this->ReadDataFromFile(ifs, tn*2+1);
    }*/
}
...
/*
  ...
  if (tn*2 < this->m_NumberOfThreadsUsed)
    ...
    this->WriteDataToFile(ofs, tn*2);
    }
  if (tn*2+1 < this->m_NumberOfThreadsUsed)
    {
    std::ofstream ofs;
    this->GetMultiThreader()->GetOfstream(ofs, tn*2+1);
    this->WriteDataToFile(ofs, tn*2+1);
    }*/
}
